...
class Agent:
    ...
    def __init__(self):
        ...
        self.finderPromptNames = {
            ...
            "exact_candidate":"findExactCandidateInYear"
        }
    ...
    def _getConversation(self):
        return json.dumps(self.message_list, indent=4)
    ...
    def doStep(self, prompt_name, prompt_args, model="llama3-70b-8192", json_mode=True):
        ...
        while continue_trying:
            try:
                ...
                f_mesg = self._extractMessage(self.llm._getGroqResponse(message_list=_f, model=model, json_mode=json_mode, temperature=0.5))
                ...
                if (trials > 2):
                    f_mesg = "Unable to generate a proper response."
                    continue_trying = False
        return f_mesg
    ...
    def resolveRequirement(self, requirement, message_list, summary):
        ...
        args = {
            "conversation":self._getConversation(),
            "summary":summary
        }
        
        key = requirement
        
        #First deal with the prompt dependencies
        ...
        #Now deal with the db dependencies
        if (key in self.dbDependencies):
            for d in self.dbDependencies[key]:
                fn_ = d["fn"]
                args_ = [self.exact_result_requirement[arg] for arg in d["args"] if arg not in args]
                for k in args:
                    if (k in d["args"]):
                        args_.append(args[k])
                _val = fn_(*args_)
        ...
        try:
            ...
            self.exact_result_requirement[key] = ""
        return
    ...
    def loop(self):
        ...
        if self.nextStep == "IntroduceYourself":
            ...
            #tool execution
            #tool response
            self._greyPrint("CURRENT STATE OF REQUIREMENT DICT: ", self.exact_result_requirement)
            ...
            choice = json.loads(self.doStep("InternalClarifier", args,  model="llama3-8b-8192"))
            ...
            if (len(no_values) > 0):
                ...
                _tool = self.doStep("PrecisionToolFinder", args, model="mixtral-8x7b-32768")
